...
# 
# di <- rbinom(n = N, size = 1, prob = 0.5)

# by replicating the vector (0,1) to the length N, then sampling without replacement, it guarantees we will have equal 1s and 0s
...
```{r}
cor.test(df$Yi_0, df$Di)

cor.test(df$Yi_1, df$Di)
```
...
```{r}
# only use one Yi because they are essentially the same?... wait idk

# (i) Yi on Di
summary(lm(Di ~ Yi_1 + Yi_0, data = df))

# (ii) Yi on Di and Xi
summary(lm(Di + Xi ~ Yi_1 + Yi_0, data = df))
```
